...
const (
	// ExtentStructsSize is the size of all extent metadata structs.
...
// ExtentEntryPair couples ExtentEntry and ExtentNode. We want to cache these
// structs in memory to avoid repeated disk reads. So this couples the entry
// with the in-memory node struct that the entry points to.
...
// Extent reprsents the ext4_extent struct in ext4. Only present in leaf nodes.
// Sorted in ascending order based on FirstFileBlock since Linux does a binary
// search on this. This points to an array of data blocks containing the file
// data. It covers `Length` data blocks starting from `StartBlock`.
type Extent struct {
	FirstFileBlock uint32
	Length         uint16
	StartBlockHi   uint16
